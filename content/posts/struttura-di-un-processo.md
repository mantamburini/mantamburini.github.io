...
Vincolo: **un documento = un processo**, e viceversa. Difficilmente sarà efficace un documento che descrive più processi (per non parlare di un processo frammentato in più documenti!). Se ci fosse questa esigenza, potrebbe essere più efficace avere un documento "indice" per i processi che si vogliono descrivere. Questo per mantenere manutenibilità dei processi separatamente, che, mano a mano che il tempo passa, avranno vite sempre più distinte; perché un processo potrebbe andare deprecato e quindi uscire dal corpus oppure un processo potrebbe modificare la sua ownership.

Ci sono una serie di informazioni, che divideremo in capitoli, che sono _imprescindibili_, che sono la condizione di "comprensibilità" e "usabilità" di un documento che descriva un processo.
...
|                Changelog | Tabella con le varie edizioni del documento           |
...
Il _versioning_ dei documenti è importante, ma esiste un comportamento non adeguato. Spesso si vedono folders pieni di documenti sorgenti (`docx` tipicamente) con la versione nel nome, "Versione 4.0 - Finale", "Versione 5.0 - Ultima rivista da DML", Versione 5.0 - DML2" etc ... Oltre ad essere straordinariamente ambiguo e difficile trovare la **vera** versione finale per davvero, è uno spreco di spazio e di energie. Personalmente ritengo che i documenti debbano essere sempre pubblicati in `PDF`, con solo il numero di versione nel nome file (solo il numero versione, ripetiamolo ancora tutti insieme) e il sorgete (`docx` o `MD` nel mio caso) è sempre solo uno che si continua a modificare. Lo storico è quindi contenuto nelle copie `PDF` versionate, ma il sorgente è uno solo che si continua a modificare. Non serve andare a modificare una versione vecchia (e se c'è, qualcuno, primo o poi lo farà).

### Scopo del documento

Questa è forse la sezione più importante, ma spesso anche la più sottovalutata o disattesa. Si possono eseguire delle operazioni perché si sta seguendo una procedura di cui si ha una consapevolezza piccola a piacere oppure si può _capire_ cosa si sta facendo. "Capire" significa avere consapevolezza dello scopo (cioè cosa si vuole ottenere) e del valore (cioè quale vantaggio porta questo processo nel momento del suo compimento).
...
Link a normative o policy aziendali interne che governano, spiegano o limitano il processo. Questi link potrebbero anche essere parte del _perché_ viene fatto un processo.
...
Se gli esecutori sono consapevoli dei KPI che sono stati concordati o, se esistono, degli SLA, lavoreranno con una consapevolezza maggiore, e tutta l'esecuzione del processo sarà più tesa al raggiungimento degli obiettivi. La consapevolezza spesso è sorprendente.
...
- Occorre formalizzare quali strumenti vengono utilizzati per fare quali cose. Da un lato un task potrebbe essere fatto solo con un certo tool, dall'altro _dovrebbe_ per questioni di sicurezza, per esempio, essere fatto con un certo tool che garantisce standard più alti, non che garantisce la fattibilità.
...
**Mai** in un documento di processo inserire password. Al massimo username, ma mai password. Non devo spiegare perché. L'azienda ha sicuramente (?) un tool a disposizione per memorizzare le password in maniera sicura. Non vogliamo raccontare qui quanti hanno sul desktop un file Word "Password.docx" o un Excel con un nome che sicuramente trarrà in inganno chiunque. No comment.
